...
- ✅ DO: Use event timestamps (`event.eventCreatedAt`)
- ❌ DON'T: Mutate `prevState`
- ❌ DON'T: Use `new Date()` or `Date.now()`
</Callout>

## Step 3: Build the Entity with Business Logic

Now let's add comprehensive business logic.

```typescript title="post.entity.ts"
...
```typescript title="post.repository.ts"
...
const createInMemoryAdapter = (): Adapter => {
  ...
  return {
    ...
    async commitEvents({ events: newEvents }) {
      ...
      console.log(`💾 Committed ${newEvents.length} event(s)`);
    }
  };
};
...
async function blogWorkflow() {
  ...
  console.log("✅ Created post:", post.entityId);
  ...
  try {
    ...
    console.log("❌ Cannot publish yet:", error.message);
  }
  console.log();
  ...
  try {
    ...
    console.log("❌ Cannot edit published post:", error.message);
  }
  console.log();

  // 7. Unpublish to make edits
  post.unpublish("Need to fix typo");
  console.log("📤 Unpublished post");
  console.log();
  ...
  console.log("✏️  Updated title while unpublished");
  console.log();

  // 9. Publish again
  post.publish("user_alice");
  console.log("🚀 Re-published with fixes");
  console.log();

  // 10. Save everything
  await postRepository.commit(post);
  console.log(`💾 Saved ${post.version} events to storage\n`);

  // 11. View event history
  console.log("📜 Event History:");
  post.events.forEach((event, i) => {
    console.log(`   ${i + 1}. ${event.eventName}`);
  });
  console.log();

  // 12. Retrieve from storage
  ...
  if (retrieved) {
    ...
    console.log("   Total events:", retrieved.version);
  }
}
...
✅ Created post: post_xyz123
   Title: Getting Started with Event Sourcing
   Is draft: true

✏️  Updated content

❌ Cannot publish yet: Cannot publish post without content
...
❌ Cannot edit published post: Cannot update published post. Unpublish it first.

📤 Unpublished post

✏️  Updated title while unpublished

🚀 Re-published with fixes

💾 Saved 6 events to storage

📜 Event History:
   1. post:created
   2. post:content_updated
   3. post:content_updated
   4. post:published
   5. post:unpublished
   6. post:content_updated
   7. post:published
...
- ✅ **Schema validation** - Type-safe events and state
- ✅ **Business logic** - Rules that prevent invalid operations
- ✅ **State management** - Pure reducers that handle events
- ✅ **Audit trail** - Complete history of all changes
- ✅ **Time travel** - Ability to reconstruct state from events
