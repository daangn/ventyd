...
describe("Entity Unit Tests", () => {
  ...
  describe("Subscribe Function", () => {
    test("should call listener when state changes", () => {
      const user = User.create({
        body: {
          nickname: "TestUser",
          email: "test@example.com",
      ...
      // Initial state - listener was not called during subscription
      expect(callCount).toBe(0);

      // Update profile - should trigger listener
      user.updateProfile({ bio: "New bio" });
      expect(callCount).toBe(1);

      // Update again
      user.updateProfile({ nickname: "UpdatedUser" });
    ...
    test("should call multiple listeners in order", () => {
      ...
      const calls: number[] = [];
      user.subscribe(() => calls.push(1));
      user.subscribe(() => calls.push(2));
      user.subscribe(() => calls.push(3));
    ...
    test("should pass current state to listener", () => {
      const user = User.create({
        body: {
          nickname: "TestUser",
          email: "test@example.com",
      ...
      user.subscribe(() => {
        capturedState = user.state;
      });

      user.updateProfile({ bio: "Software Engineer" });

      expect(capturedState).toBeDefined();
      expect(capturedState?.bio).toBe("Software Engineer");
      expect(capturedState?.nickname).toBe("TestUser");
    });

    test("should not call listener when entity is loaded from events", () => {
      const events = [
        {
          eventId: "evt-1",
          eventName: "user:created" as const,
      ...
      // Loading from events should not trigger listeners
      expect(callCount).toBe(0);
    });

    test("should call listener for each event in a sequence", () => {
      const user = User.create({
        body: {
          nickname: "TestUser",
          email: "test@example.com",
    ...
    test("should allow listener to access updated state", () => {
      const user = User.create({
        body: {
          nickname: "TestUser",
          email: "test@example.com",
      ...
      user.subscribe(() => {
        stateSnapshots.push(user.state.bio ?? "no bio");
      });

      user.updateProfile({ bio: "First bio" });
      user.updateProfile({ bio: "Second bio" });
      user.updateProfile({ bio: "Third bio" });

      expect(stateSnapshots).toEqual(["First bio", "Second bio", "Third bio"]);
    });

    test("should work with Order entity", () => {
      const order = Order.create({
        body: {
          customerId: "cust-123",
    ...
    test("should not be affected by listener errors", () => {
      const user = User.create({
        body: {
          nickname: "TestUser",
          email: "test@example.com",
      ...
      // First listener throws, but second should still be called
      expect(() => {
        user.updateProfile({ bio: "New bio" });
    ...
    test("should unsubscribe listener when dispose function is called", () => {
      ...
      // Update should not trigger listener after unsubscribe
      user.updateProfile({ bio: "Second bio" });
      expect(callCount).toBe(1);
    });

    test("should unsubscribe only the specific listener", () => {
      const user = User.create({
        body: {
          nickname: "TestUser",
          email: "test@example.com",
      ...
      const unsubscribe1 = user.subscribe(() => {
        callCount1++;
      });
      const unsubscribe2 = user.subscribe(() => {
        callCount2++;
      });
      user.subscribe(() => {
        callCount3++;
      ...
      // Only second and third listeners should be called
      user.updateProfile({ bio: "Second bio" });
      expect(callCount1).toBe(1);
      ...
      // Only third listener should be called
      user.updateProfile({ bio: "Third bio" });
      expect(callCount1).toBe(1);
      expect(callCount2).toBe(2);
      expect(callCount3).toBe(3);
    });
